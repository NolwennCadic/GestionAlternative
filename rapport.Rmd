...
```
Régression de la rentabilité de P10 par rapport au modèle du MEDAF. Celui-ci n'est pas significativement différent de 0 à $95\%$. Notre portefeuille ne dégage donc pas de rentabilité supérieure au marché. La stratégie momentum choisie n'est pas plus efficace qu'une gestion passive du portefeuille de marché.
...
```{r echo = FALSE}
print(SimpleMeasures[, c(1, 5) ])
```
On observe que $T_{P_{P10\_P1}} > T_{P_{M}}$. Notre protefeuille performerait mieux que celui du marché selon cette mesure (contrairement au résultat obtenu par la mesure du ratio de Sharpe). Notre protefeuille n'est peut être pas bien diversifié, dans ce cas le ratio de Treynor est plus pertinent. Notre protefeuille réalise bien un excès de rentabilité par rapport au marché, donc notre stratégie momentum 6 mois - 12 mois est une startégie dégageant de la rentabilité.
...
print(SimpleSummaryP10MinusP1)
```
Régression de la rentabilité de P10 par rapport au modèle du MEDAF. Au seuil de $95\%$
